...
basic発動時はこのAnimationが攻撃速度に対応したフレーム数（`B = 40 / A`）になるように倍速再生される。例として攻撃速度（=白字攻撃速度+緑字攻撃速度）が3のウチの場合、`B = 40 / 3 = 13.333...` で、`basic前隙`は `((40 / 3) / 42) * 25 = 7.937F`、同様に `basic持続` は `0.317F`、`basic後隙` は `5.079F` となる。
