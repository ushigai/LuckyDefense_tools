...
# Internal action names requested by spec.
ACTION_BASIC = "basic"
...
def _validate_params(p: LancelotParams5003) -> None:
    if p.attack_speed <= 0:
    ...
    if p.additional_dmg < 0:
        raise ValueError("additional_dmg must be >= 0")
    ...
    if p.skill1_rate + p.skill2_rate > 100 + 1e-12:
...
def _choose_nonult_action(rng: random.Random, p: LancelotParams5003) -> str:
    r = rng.random() * 100.0
    if r < p.skill1_rate:
...
def _damage_for_action(action: str, p: LancelotParams5003, rng: random.Random) -> float:
    add = 0.0
    if action == ACTION_BASIC:
        base_no_add = p.attack_power * p.base_attack_mult
    elif action == ACTION_SKILL1:
        base_no_add = p.attack_power * p.skill1_mult
        add = p.additional_dmg
    elif action == ACTION_SKILL2:
        base_no_add = p.attack_power * p.skill2_mult
        add = p.additional_dmg
    elif action == ACTION_ULT:
        base_no_add = p.attack_power * p.ult_mult
        add = p.additional_dmg * 10.0
    else:
        raise ValueError(f"unknown action: {action}")

    crit_mult = p.crit_dmg if _roll_percent(rng, p.crit_rate) else 1.0
    return (base_no_add * crit_mult) + add
...
def mean_total_damage_5003(options: Dict[str, Any]) -> Tuple[float, float, float, float, float]:
    """
    Return (basic, skill1, skill2, skill3, ult) mean total damage.
    """
    params = LancelotParams5003(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Lancelot(5003) Monte-Carlo damage simulator")
    ...
    print("=== Lancelot(5003) Monte-Carlo Result ===")
